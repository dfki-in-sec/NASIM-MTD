...
class NASimEnv(gym.Env):
    ...
    steps : int
        ...
        generative steps)
    current_time : int
        the passed time for Moving Target Defence #MT-T
    """
    ...
    def __init__(self,
        ...
        self.steps = 0
        self.current_time = 0 #MT-T
        self.mt_mapping = {} #MT-T
        self.default_mapping() #MT-T
        self.honeypot_hit = False #MT-T2
        self.one_goal = one_goal #MT-T5
    ...
    def reset(self):
        ...
        self.default_mapping() #MT-T
        self.honeypot_hit = False #MT-T2
        #self.current_state = self.network.reset(self.current_state) CPB4
        self.current_state = State.generate_initial_state(self.network)
    ...
    def default_mapping(self):
        # ingnore subnet 0 = internet
        for i in range(2, self.network.get_number_of_subnets()):
            # find all hosts in one subnet
            subnet_hosts = []
            for t in self.network.address_space:
                if t[0] == i:
                    host = self.current_state.get_host(t)
                    #if host.os['Penbox'] != 1.0:
                        #subnet_hosts.append(t)
            if len(subnet_hosts) > 1:
                mapping_dict = {}
                for j in range(len(subnet_hosts)):
                    mapping_dict[subnet_hosts[j]] = subnet_hosts[j]
                self.mt_mapping[i] = mapping_dict
    ...
    def generative_step(self, state, action):
        ...
        #MT-T
        self.current_time = self.current_time + action.cost
        if self.scenario.movement_time is not None:
            if self.current_time > self.scenario.movement_time:

                state = self.moving_target(state.copy())
                self.network.update_special_hosts_lists(self.mt_mapping)
                self.current_time = self.current_time - self.scenario.movement_time
        ...
        #MT-T2
        self.honeypot_attacked(action, next_state) 
        if not done: 
            done = self.honeypot_hit

        reward = action_obs.value - action.cost
        return next_state, obs, reward, done, action_obs.info()

    #MT-T
    def moving_target(self, new_state):
        """

        Performs Moving Target Defence on Host_Addresses in Subnets with more than one Host 

        Parameters
        ----------
        state : State
            The state to perform the MT in

        Returns
        -------
        State
            the state after MT was performed
        
        """

        # ingnore subnet 0 = internet and 1 with only penbox
        for i in range(2, self.network.get_number_of_subnets()):
            # find all hosts in one subnet
            subnet_hosts = []
            for t in self.network.address_space:
                if t[0] == i:
                    host = new_state.get_host(t)
                    if host.os['Penbox'] != 1.0:
                        subnet_hosts.append(t)
            if len(subnet_hosts) > 1:
                shuffled_list = sample(subnet_hosts, len(subnet_hosts))
                vector_list = []
                mapping_dict = {}
                for j in range(len(subnet_hosts)):
                    # change vector
                    vector_list.append(new_state.get_host(subnet_hosts[j]).copy())
                    # change address
                    vector_list[j].change_host_address(shuffled_list[j])
                # update
                for j in range(len(subnet_hosts)):
                    new_state.update_host(shuffled_list[j], vector_list[j])
                    #make current local dict
                    mapping_dict[subnet_hosts[j]] = shuffled_list[j]
                # make dict for start id
                if self.mt_mapping == {}:
                    self.mt_mapping = mapping_dict
                for key, value in self.mt_mapping.items():
                    self.mt_mapping[key] = mapping_dict[value]
    ...
    def goal_reached(self, state=None):
        ...
        if one_goal=true: the goal state is when one sensitive host have been compromised.
        ...
        if state is None:
            state = self.current_state
        if self.one_goal: #MT-T5
            return self.network.one_sensitive_host_compromised(state)
        return self.network.all_sensitive_hosts_compromised(state)      

    #MT-T2
    def honeypot_attacked(self, action, state=None):
        """Check if the state has compromised/attacked Honeypot.

        Only one honeypot needs to be compromised.

        Parameters
        ----------
        action : Action
            the action to check if it was performed against honeypot
        ...
        -------
        bool
            True if state has compromised (on user level) honeypot, otherwise False.
        """
        if state is None:
            state = self.current_state
        # check if honepot is compromised
        self.honeypot_hit = self.network.honeypot_compromised(state)
        # check if attack against honeypot (even if unsuccessful)
        # if not self.honeypot_hit:
        #     if not action.is_scan(): 
        #         self.honeypot_hit = self.network.is_honeypot(action.target)
