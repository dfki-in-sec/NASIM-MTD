...
def load_action_list(scenario):
    ...
    for address in scenario.address_space:
        ...
        action_list.append(                               #CBP
            VulScan(address, scenario.vul_scan_cost)      #CBP
        )                                                 #CBP
        action_list.append(
            OSScan(address, scenario.os_scan_cost)
        )
        action_list.append(                                #CBP3
            Wiretapping(address, scenario.wiretapping_cost)#CBP3
        )                                                  #CBP3
        action_list.append(
...
class Action:
    ...
    def is_scan(self):
        ...
        """
        return isinstance(self, (ServiceScan, OSScan, SubnetScan, ProcessScan, VulScan, Wiretapping)) #CBP
    ...
    def is_remote(self):
        ...
        """
        return isinstance(self, (ServiceScan, OSScan, Exploit, Wiretapping)) #CBP
...
class Exploit(Action):
    ...
    def __str__(self):
        return (f"{super().__str__()}, os={self.os}, vul={self.vul}, c_needed= {self.credentials_needed}, "
...
class PrivilegeEscalation(Action):
    ...
    def __init__(self,
        ...
        super().__init__(name=name,
                         ...
                         prob=prob,
                         req_access=req_access,
                         credentials_tofind=credentials_tofind) #CBP4
        self.access = access
        self.os = os
        self.process = process
        self.credentials_tofind = credentials_tofind #CBP4

    def __str__(self):
        return (f"{super().__str__()}, os={self.os}, c_tofind= {self.credentials_tofind} " #CBP4
...
class ActionResult:
    ...
    def __init__(self,
        ...
        self.vul = {} if vul is None else vul #CBP
        self.os = {} if os is None else os
        self.c_tofind = {} if c_tofind == 0 else c_tofind #CBP3
        self.c_found = {} if c_found == 0 else c_found  # CBP3
        self.c_needed = {} if c_needed == 0 else c_needed  # CBP3
    ...
    def info(self):
        ...
        return dict(
            ...
            vul=self.vul, #CBP
            os=self.os,
            c_tofind=self.c_tofind, #CBP3
            c_found=self.c_found,  # CBP3
            c_needed=self.c_needed,  # CBP3
...
class ParameterisedActionSpace(spaces.MultiDiscrete):
    ...
    def get_action(self, action_vec):
        ...
        if a_class == Exploit:
            ...
            vul = self.scenario.vul[action_vec[5]] # TODO CHECK
            c_needed = self.scenario.credentials[action_vec[6]] # TODO CHECK
            a_def = self._get_exploit_def(service, os, vul, c_needed) #CBP2
        else:
            # privilege escalation
            # have to make sure it is valid choice
            # and also get constant params (name, cost, prob, access)
            proc = self.scenario.processes[action_vec[6]] # TODO CHECK
    ...
    def _get_exploit_def(self, service, os, vul, c_needed): #CBP2
        ...
        if os not in e_map[service]:
            return None
        if vul not in e_map[service][os]: #CBP2
            return None #CBP2
        if c_needed not in e_map[service][os][vul]: #CBP3
            return None #CBP3
        return e_map[service][os][vul][c_needed] #CBP 2+3
