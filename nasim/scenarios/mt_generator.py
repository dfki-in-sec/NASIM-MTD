...
# Percentages (chance of exploit/privesc containing cred/vulns)
VULN_PERCENTAGE = .4
CREDS_PERCENTAGE = .1

class MTScenarioGenerator:
    ...
    **Host Configuration distribution**:
    ...
    def generate(self,
                 ...
                 subnet_scan_cost=1,
                 process_scan_cost=1,
                 vul_scan_cost=1, #MT-T4
                 wiretapping_cost=1, #MT-T4
                 uniform=False,
                 alpha_H=2.0,
                 alpha_V=2.0,
                 lambda_V=1.0,
        ...
        num_sensitive : int, optional
            number of sensitive hosts in network - must be at least 1
        num_honeypots : int, optional
            number of honeypots in network
        ...
        num_vulns : int, optional
            number of vulnerabilities to create (default=1) (cannot be 0/None)
        num_creds : int, optional
            number of credentials to find, only 8 possible (default=None) (cannot be 0)
        r_sensitive : float, optional
            reward for sensitive subnet documents (default=10)
        r_honeypot : float, optional
            cost for exploiting honeypot (default=-10)
        ...
        vul_scan_cost : int or float, optional
            cost for a vulnerability scan (default=1)
        wiretapping_cost : int or float, optional
            cost for a wiretapping (default=1)
        ...
        addresses : int, optional
            number of addresses in subnet (2), if None then only num_hosts - everything more will be filled with empty hosts
        ...
        assert 0 < num_os
        assert 0 < r_sensitive
        assert 0 < num_sensitive
        assert num_sensitive + num_honeypots <= num_hosts
        assert 0 < alpha_H and 0 < alpha_V and 0 < lambda_V
        if addresses != None:
            assert num_hosts < addresses
        ...
        self._generate_vulnerabilities(num_vulns)
        self._generate_credentials(num_creds)
        self._generate_exploits(num_exploits, exploit_cost, exploit_probs, num_creds, num_vulns)
        self._generate_privescs(num_privescs, privesc_cost, privesc_probs, num_creds)
        self._generate_sensitive_hosts(r_sensitive, num_sensitive)
        self._generate_honeypot_hosts(r_honeypot, num_honeypots)
        self.base_host_value = base_host_value
        self.host_discovery_value = host_discovery_value
        if uniform:                                                             
            self._generate_uniform_hosts(num_creds)
        else:
            self._generate_correlated_hosts(alpha_H, alpha_V, lambda_V, num_creds)
        ...
        self.vul_scan_cost = vul_scan_cost #MT-T4
        self.wiretapping_cost = wiretapping_cost #MT-T4
        ...
        self.movement_time = movement_time #MT-T

        self._generate_penbox_host()
        
        # fill up addresses with empty hosts
        if addresses != None:
            self._generate_empty_hosts(addresses)
    ...
    def _construct_scenario(self):
        ...
        scenario_dict[u.VUL] = self.vulnerabilities
        ...
        scenario_dict[u.VUL_SCAN_COST] = self.vul_scan_cost #MT-T4
        scenario_dict[u.WIRETAPPING_COST] = self.wiretapping_cost #MT-T4
        scenario_dict[u.FIREWALL] = self.firewall
        scenario_dict[u.HOSTS] = self.hosts
        scenario_dict[u.STEP_LIMIT] = self.step_limit
        scenario_dict[u.MOVEMENT_TIME] = self.movement_time #MT-T
        if len(self.honeypots_hosts.keys()) > 0:
            scenario_dict[u.HONEYPOTS] = self.honeypots_hosts
        scenario = Scenario(
            scenario_dict, name=self.name, generated=True
        )
        return scenario

    # need only 1 subnet for test
    def _generate_subnets(self, num_hosts):
        # first ist penbox, second is all hosts
        subnets = [0, 1]
        subnets.append(num_hosts)
        self.subnets = subnets 

    def _generate_topology(self):
        # including internet subnet
        num_subnets = len(self.subnets)
        topology = np.zeros((num_subnets+1, num_subnets+1))
        # only one subnet - no restrictions, but internet
        for row in range(num_subnets + 1):
            for col in range(num_subnets + 1):
                if row == u.INTERNET and col > DMZ:
                    continue
                if row > DMZ and col == u.INTERNET:
                    continue
                topology[row][col] = 1
        if num_subnets == num_subnets + 1:
    ...
    def _generate_os(self, num_os):
        ...
        self.os.append("Penbox")

    def _generate_services(self, num_services):
        self.services = [f"srv_{s}" for s in range(num_services)]
        self.services.append("init")

    def _generate_processes(self, num_processes):
        self.processes = [f"proc_{s}" for s in range(num_processes)]

    def _generate_vulnerabilities(self, num_vulns):
        if num_vulns != None:
            self.vulnerabilities = [f"vuln_{s}" for s in range(num_vulns)]
        else:
            self.vulnerabilities = []
    
    def _generate_credentials(self, num_creds):
        if num_creds != None:
            self.credentials = list(range(1, num_creds+1))

    def _generate_exploits(self, num_exploits, exploit_cost, exploit_probs, num_creds, num_vulns):
        exploits = {}
        ...
        # add None since some exploits might work without vuln
        if num_vulns != None:
            possible_vulns = self.vulnerabilities + [None]
        else:
            possible_vulns = [None, None]
        vulns_perc = [VULN_PERCENTAGE/(len(possible_vulns)-1) for x in range(len(possible_vulns)-1)] + [1.0-VULN_PERCENTAGE]
        # add None since some exploits might work without creds
        if num_creds != None:
            possible_creds = self.credentials + [0]
        else:
            possible_creds = [0, 0]
        creds_perc = [CREDS_PERCENTAGE/(len(possible_creds)-1) for x in range(len(possible_creds)-1)] + [1.0-CREDS_PERCENTAGE]
        # we create one exploit per service
        exploits_added = 0
        while exploits_added < num_exploits:
            srv = np.random.choice([x for x in self.services if x != "init"])
            os = np.random.choice([x for x in possible_os if x != "Penbox"])
            vuln = np.random.choice(possible_vulns, p = vulns_perc)
            cred = np.random.choice(possible_creds, p = creds_perc)
            ...
            if e_name not in exploits:
                exploits[e_name] = {
                    ...
                    u.EXPLOIT_ACCESS: al,
                    u.EXPLOIT_VUL: vuln,
                    u.EXPLOIT_CREDENTIALS_NEEDED: cred
                }
                exploits_added += 1
        exploits["e_init"] = {
            u.EXPLOIT_SERVICE: "init",
            u.EXPLOIT_OS: "Penbox",
            u.EXPLOIT_PROB: 1.0,
            u.EXPLOIT_COST: 0,
            u.EXPLOIT_ACCESS: u.ROOT_ACCESS,
            u.EXPLOIT_VUL: None,
            u.EXPLOIT_CREDENTIALS_NEEDED: 0
        }
        self.exploits = exploits

    def _generate_privescs(self, num_privesc, privesc_cost, privesc_probs, num_creds):
        privescs = {}
        privesc_probs = self._get_action_probs(num_privesc, privesc_probs)
        # add None since some privesc might work for all OS
        possible_os = self.os + [None]
        possible_os.remove("Penbox")
        # add None since some exploits might work without creds
        if num_creds != None:
            possible_creds = self.credentials + [0]
        else:
            possible_creds = [0, 0]
        creds_perc = [CREDS_PERCENTAGE/(len(possible_creds)-1) for x in range(len(possible_creds)-1)] + [1-CREDS_PERCENTAGE]
        ...
        while privescs_added < num_privesc:
            ...
            if pe_name not in privescs:
                privescs[pe_name] = {
                    ...
                    u.PRIVESC_CREDENTIALS_TOFIND: cred
                }
                privescs_added += 1
    ...
    # generate Penbox + add os + exploit
    def _generate_penbox_host(self):  
        os_cfg = self._convert_to_os_map("Penbox")  

        srv_config_set, proc_config_set, vuln_config_set = self._possible_host_configs()
        num_srv_configs = len(srv_config_set)
        num_proc_configs = len(proc_config_set)
        num_vuln_configs = len(vuln_config_set)
        ...
        if(num_vuln_configs > 0):
            vuln_cfg = vuln_config_set[np.random.choice(num_vuln_configs)]
            vuln_cfg = self._convert_to_vulnerability_map(vuln_cfg)
        else:
            vuln_cfg = {}

        new_srv = srv_cfg.copy()
        for key in new_srv.keys():
            if key == "init":
                new_srv[key] = True
            else:
                new_srv[key] = False
        new_proc = proc_cfg.copy()
        for key in new_proc.keys():
            new_proc[key] = False
        if vuln_cfg != {}:
            new_vuln = vuln_cfg.copy()
            for key in new_vuln.keys():
                new_vuln[key] = False
        else:
            new_vuln = {}
        ...
        host = Host(
            ...
            firewall={},
            vul=new_vuln.copy(),
            c_tofind=self.hosts[(1,0)].c_tofind
        )
        self.hosts[(1,0)] = host

    def _generate_sensitive_hosts(self, r_sensitive, num_sensitive):
        sensitive_hosts = {}
        
        i = 0
        while i < num_sensitive:
            host = np.random.randint(0, self.subnets[SENSITIVE])
            if (SENSITIVE, host) in sensitive_hosts:
                continue
            else:
                sensitive_hosts[(SENSITIVE, host)] = r_sensitive
                i += 1

        self.sensitive_hosts = sensitive_hosts
    
    def _generate_honeypot_hosts(self, r_honeypots, num_honeypots):
        honeypots_hosts = {}
        
        i = 0
        while i < num_honeypots:
            host = np.random.randint(0, self.subnets[SENSITIVE])
            if (SENSITIVE, host) in honeypots_hosts or self._is_sensitive_host((SENSITIVE, host)):
                continue
            else:
                honeypots_hosts[(SENSITIVE, host)] = r_honeypots
                i += 1

        self.honeypots_hosts = honeypots_hosts

    def _generate_uniform_hosts(self, num_creds):
        hosts = dict()
        srv_config_set, proc_config_set, vuln_config_set = self._possible_host_configs()
        num_srv_configs = len(srv_config_set)
        num_proc_configs = len(proc_config_set)
        ...
        for subnet, size in enumerate(self.subnets):
            ...
            for h in range(size):
                ...
                os = np.random.choice([x for x in self.os if x != "Penbox"])
    ...
    def _possible_host_configs(self):
        """Generate set of all possible host service and process and vulnerabilities configurations
        ...
        list[list]
            ...
            processes
        list[list]
            all possible vulnerability configurations, same as above except for
            processes
        """
        ...
        vuln_configs = self._permutations(len(self.vulnerabilities))[:-1]
        return srv_configs, proc_configs, vuln_configs
    ...
    def _generate_correlated_hosts(self, alpha_H, alpha_V, lambda_V, num_creds):
        hosts = dict()
        prev_configs = []
        prev_os = []
        prev_srvs = []
        ...
        # add 0 since some exploits might work without creds
        if num_creds != None:
            possible_creds = self.credentials + [0]
        else:
            possible_creds = [0, 0]
        ...
        for subnet, size in enumerate(self.subnets):
            if subnet == u.INTERNET:
                continue
            for m in range(size):
                os, services, processes, vulnerabilities = self._get_host_config(
                    host_num,
                    alpha_H,
                    prev_configs,
                    alpha_V,
                ...
                vuln_cfg = self._convert_to_vulnerability_map(vulnerabilities)
                cred = np.random.choice(possible_creds, p = creds_perc)
                host_num += 1
                address = (subnet, m)
                value = self._get_host_value(address)
        ...
        if host_num == 0 \
           or np.random.rand() < (alpha_H / (alpha_H + host_num - 1)):
            ...
            new_config = self._sample_config(
                alpha_V, prev_srvs, lambda_V, prev_os, prev_procs, prev_vulns
            )
        else:
    ...
    def _sample_config(self,
        ...
        for os in self.os:
            if os != "Penbox":
                possible_os.append(os)
        os = self._dirichlet_sample(
            alpha_V, possible_os, prev_os
        )
        ...
        if self.vulnerabilities != []:
            new__vulnerability_cfg = self._dirichlet_process(
                alpha_V, lambda_V, len(self.vulnerabilities), prev_vulns
            )
        else:
            new__vulnerability_cfg = {}

        return os, new_services_cfg, new_process_cfg, new__vulnerability_cfg
    ...
    def _is_honeypot_host(self, addr):
        return addr in self.honeypots_hosts
    ...
    def _convert_to_service_map(self, config):
        ...
        for srv, val in zip(self.services, config):
            if srv == "init": #init service only for first box
                service_map[srv] = False
            else:
                service_map[srv] = val
        return service_map
    ...
    def _convert_to_vulnerability_map(self, config):
        """Converts list of bools to a map from vuln name -> bool """
        vulnerability_map = {}
        for vuln, val in zip(self.vulnerabilities, config):
            vulnerability_map[vuln] = val
    ...
    def _ensure_host_vulnerability(self):
        """Ensures each subnet has at least one vulnerable host and all sensitive + honeypots hosts
        are vulnerable
        """
        vulnerable_subnets = set()
        for host_addr, host in self.hosts.items():
            if not self._is_sensitive_host(host_addr) \
               and not self._is_honeypot_host(host_addr) \
               and host_addr[0] in vulnerable_subnets:
                continue

            if self._is_sensitive_host(host_addr) or self._is_honeypot_host(host_addr):
        ...
        # make sure all necessary credentials can be found
        creds_needed = []
        for host_addr, host in self.hosts.items():
            if host.c_needed != 0 and host.c_needed not in creds_needed:
                creds_needed.append(host.c_needed)

        possible_hosts = []
        for host_addr, host in self.hosts.items():
            if not self._is_sensitive_host((2, host_addr)) and not self._is_honeypot_host((2, host_addr)):
                if self._host_is_vulnerable(host, access_level=u.ROOT_ACCESS):
                    possible_hosts.append(host_addr)
        if (1,0) in possible_hosts: #creds must come from same subnet
            possible_hosts.remove((1,0))
        #print("Possible Hosts: " + str(possible_hosts))

        while len(creds_needed) > 0:
            i = np.random.randint(0, len(possible_hosts))
            j = np.random.randint(0, len(creds_needed))
            c = creds_needed[j]
            host = self.hosts[possible_hosts[i]]
            if host.c_needed != c:
                host.c_tofind = host.c_tofind*10 + c
                creds_needed.remove(c)

        #for host_addr, host in self.hosts.items():
        #    print(host_addr, end=" ")
        #    print(str(self._host_is_vulnerable(host, access_level=u.ROOT_ACCESS)))
    ...
    def _host_is_vulnerable_to_exploit(self, host, exploit_def):
        ...
        e_vuln = exploit_def[u.EXPLOIT_VUL]
        e_cred = exploit_def[u.EXPLOIT_CREDENTIALS_NEEDED]
        if not host.services[e_srv]:
            return False
        if (e_vuln is None or host.vul[e_vuln]) and (e_os is None or host.os[e_os]):
            host.c_needed = e_cred
            return True
    ...
    def _update_host_exploit_vulnerability(self, host, os_constraint):
        ...
        e_init = self.exploits["e_init"]
        if e_init in valid_e:
            valid_e.remove(e_init)

        e_def = np.random.choice(valid_e)
        host.services[e_def[u.EXPLOIT_SERVICE]] = True
        host.vul[e_def[u.EXPLOIT_VUL]] = True
        host.c_needed = e_def[u.EXPLOIT_CREDENTIALS_NEEDED]
    ...
    def _get_host_value(self, address):
        if address in self.honeypots_hosts:
            return float(self.honeypots_hosts.get(address))
        return float(self.sensitive_hosts.get(address, self.base_host_value))

    def _generate_firewall(self):
        ...
        for src in range(0, 2):
            for dest in range(0, 2):
                if src != dest:
                    firewall[(src, dest)] = self.services

        for src in range(1, num_subnets):
            for dest in range(1, num_subnets):
                if src != dest:
                    firewall[(src, dest)] = self.services
                
        self.firewall = firewall

    # create empty hosts to fill up address space
    def _generate_empty_hosts(self, addresses):

        # get last config, which is all False
        srv_cfg = self._permutations(len(self.services))[-1]
        proc_cfg = self._permutations(len(self.processes))[-1]
        
        srv_cfg = self._convert_to_service_map(srv_cfg)
        proc_cfg = self._convert_to_process_map(proc_cfg)

        if(len(self.vulnerabilities) > 0):
            vuln_cfg = self._permutations(len(self.vulnerabilities))[-1]
            vuln_cfg = self._convert_to_vulnerability_map(vuln_cfg)
        else:
            vuln_cfg = {}
        ...
        subnet = 2
        for h in range(self.subnets[subnet], addresses):
            address = (subnet, h)
