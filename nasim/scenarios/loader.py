...
OPTIONAL_CONFIG_KEYS = {
    u.STEP_LIMIT: int,
    u.MOVEMENT_TIME: int, #MT-T
    u.HONEYPOTS: dict #MT-T2
}
...
PRIVESC_KEYS = {
    ...
    u.PRIVESC_CREDENTIALS_TOFIND: int #CBP3
}
...
class ScenarioLoader:
    ...
    def load(self, file_path, name=None):
        ...
        self._parse_movement_time() #MT-T
    ...
    def _construct_scenario(self):
        ...
        scenario_dict[u.VUL_SCAN_COST] = self.vul_scan_cost #CBP
        scenario_dict[u.WIRETAPPING_COST] = self.wiretapping_cost #CBP3
    ...
    def _parse_honeypots(self):
        if u.HONEYPOTS not in self.yaml_dict:
            self.honeypots = None
        else:
            honeypots = self.yaml_dict[u.HONEYPOTS]
            self._validate_honeypots(honeypots)

            self.honeypots = dict()
            for address, value in honeypots.items():
                self.honeypots[eval(address)] = value
    ...
    def _validate_honeypots(self, honeypots):
        # check honeypots is valid dict of (subnet, id) : value
        assert len(honeypots) > 0, \
            ("Number of honeypots must be >= 1: "
             f"{len(honeypots)} not >= 1")

        assert len(honeypots) <= self.num_hosts, \
            ("Number of honeypots must be <= total number of "
             f"hosts: {len(honeypots)} not <= {self.num_hosts}")

        # honeypots must be valid address
        for address, value in honeypots.items():
            subnet_id, host_id = eval(address)
            assert self._is_valid_subnet_ID(subnet_id), \
                ("Invalid honeypots tuple: subnet_id must be a valid"
                 f" subnet: {subnet_id} != non-negative int less than "
                 f"{len(self.subnets) + 1}")

            assert self._is_valid_host_address(subnet_id, host_id), \
                ("Invalid honeypots tuple: host_id must be a valid"
                 f" int: {host_id} != non-negative int less than"
                 f" {self.subnets[subnet_id]}")

            assert isinstance(value, (float, int)) and value < 0, \
                (f"Invalid honeypots tuple: invalid value: {value}"
                 f" != a negative int or float")

        # 5.c honeypots must not contain duplicate addresses
        for i, m in enumerate(honeypots.keys()):
            h1_addr = eval(m)
            for j, n in enumerate(honeypots.keys()):
                if i == j:
                    continue
                h2_addr = eval(n)
                assert h1_addr != h2_addr, \
                    ("Honeypots list must not contain duplicate host "
                     f"addresses: {m} == {n}")
    ...
    def _validate_host_config(self, addr, cfg):
        ...
        #CBP
        host_vul = cfg[u.HOST_VUL]
        for service in host_vul:
            assert service in self.vul, \
                (f"{err_prefix} Invalid service in configuration vul "
                 f"list: {service}")

        assert len(host_vul) == len(set(host_vul)), \
    ...
    def _parse_hosts(self):
        ...
        for address, h_cfg in self.host_configs.items():
            ...
            #CBP
            os_cfg, srv_cfg, vul_cfg, proc_cfg,c_needed_cfg, c_tofind_cfg = self._construct_host_config(h_cfg)
            value = self._get_host_value(formatted_address, h_cfg)
            hosts[formatted_address] = Host(
                address=formatted_address,
                os=os_cfg,
                services=srv_cfg, #CBP
                vul=vul_cfg,
                c_tofind=c_tofind_cfg, #CBP3
                c_needed=c_needed_cfg, #CBP3
                processes=proc_cfg,
    ...
    def _construct_host_config(self, host_cfg):
        ...
        #CBP start
        vul_cfg = {}
        for service in self.vul:
            vul_cfg[service] = service in host_cfg[u.HOST_VUL]
        #CBP stop

        #CBP3
        c_needed_cfg = host_cfg[u.HOST_CREDENTIALS_NEEDED]

        c_tofind_cfg = host_cfg[u.HOST_CREDENTIALS_TOFIND]

        processes_cfg = {}
        for process in self.processes:
            processes_cfg[process] = process in host_cfg[u.HOST_PROCESSES]
        return os_cfg, services_cfg, vul_cfg, processes_cfg, c_needed_cfg, c_tofind_cfg  #CBP

    def _get_host_value(self, address, host_cfg):
        if address in self.sensitive_hosts:
            return float(self.sensitive_hosts[address])
        if self.honeypots != None and address in self.honeypots: #MT-T2
            return float(self.honeypots[address])
    ...
    #MT-T
    def _parse_movement_time(self):
        if u.MOVEMENT_TIME not in self.yaml_dict:
            movement_time = None
        else:
            movement_time = self.yaml_dict[u.MOVEMENT_TIME]
            assert movement_time > 0, \
                f"Step limit must be positive int: {movement_time} is invalid"
