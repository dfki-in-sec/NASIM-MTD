...
class DQNAgent:
    ...
    def __init__(self,
        ...
        # logger setup
        self.logger = SummaryWriter(comment=str(lr) + "_" + str(batch_size) + "_" + str(replay_size) + "_" + str(final_epsilon) + "_" + str(exploration_steps) + "_" + str(gamma) + "_" + str(hidden_sizes) + "_" + str(target_update_freq))
...
if __name__ == "__main__":
                            ...
                            for gamma in gamma_list:
                                for hidden in hidden_sizes:
                                    for target in target_update_freq:
                                        if i == 8:
                                            i = 0
                                        para_list[i].append([lr, batch, relay, final_epsilon, exploration_steps, gamma, hidden, target])
                                        i += 1

        para_list = para_list[0] #For pc
        ...
        for file in os.listdir(directory):
            if file.endswith('.npy'):
                filex = file.replace(".", "_")
                para = filex.split("_", )
                para_list.remove([para[0],para[1],para[2],para[3],para[4],para[5],para[6],para[7]])
        with Pool(12) as p:
            p.map(run, para_list)
