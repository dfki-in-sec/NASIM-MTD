...
from keras.optimizers import adam_v2, rmsprop_v2  #changed
...
def OurModel(input_shape, action_space, lr):
    ...
    Actor.compile(loss='categorical_crossentropy', optimizer=rmsprop_v2(lr=lr))

    Critic = Model(inputs=X_input, outputs=value)
    Critic.compile(loss='mse', optimizer=rmsprop_v2(lr=lr))
...
class A3CAgent:
    ...
    def __init__(self, env_name):
        ...
        self.env = nasim.load("../scenarios/benchmark/"+env_name, flat_actions=True, flat_obs=True) #CBP
    ...
    def train(self, n_threads):
        ...
        envs = [nasim.load("../scenarios/benchmark/"+env_name, flat_actions=True, flat_obs=True) for i in range(n_threads)]
